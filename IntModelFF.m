...
% IntModelFF
%
% Solves for LQ optimal policy in Curdia and Woodford (2010)
%
% In this version the nat variables are defined as excluding:
%   - dynamic effects of financial frictions
%   - markup shock
%   - tax rate shock
%
% Usage:
...
%% Check arguments
if ismember('PersLevel_0',varargin), PersLevel = 0; end
if ismember('PersLevel_50',varargin), PersLevel = 50; end
if ismember('PersLevel_80',varargin), PersLevel = 80; end
if ismember('PersLevel_85',varargin), PersLevel = 85; end
if ismember('PersLevel_86',varargin), PersLevel = 86; end
if ismember('PersLevel_87',varargin), PersLevel = 87; end
if ismember('PersLevel_88',varargin), PersLevel = 88; end
if ismember('PersLevel_89',varargin), PersLevel = 89; end
if ismember('PersLevel_90',varargin), PersLevel = 90; end
if ismember('PersLevel_91',varargin), PersLevel = 91; end
if ismember('PersLevel_95',varargin), PersLevel = 95; end
if ismember('PersLevel_99',varargin), PersLevel = 99; end
if ismember('PersLevel_90_99',varargin), PersLevel = 90; PersLevelAlt = 99; end
if ismember('SigmaRatio_1',varargin), SigmaRatio = 1; end
if ismember('SigmaRatio_2',varargin), SigmaRatio = 2; end
if ismember('SigmaRatio_5',varargin), SigmaRatio = 5; end
if ismember('SigmaRatio_10',varargin), SigmaRatio = 10; end
if ismember('eta_1',varargin), eta = 1; end
if ismember('eta_5',varargin), eta = 5; end
if ismember('eta_50',varargin), eta = 50; end
...
%% Exercise type
ExerciseName = sprintf('FF_PersLevel_%02.0f_SigmaRatio_%02.0f_eta_%02.0f',PersLevel,SigmaRatio,eta);
...
rho_b = 3.2;
% elast_b_xi_i = -0.1*4;
% elast_omega_b_TGT = ElastSPb/400; 
% elast_omega_b = elast_omega_b_TGT; 
...
nF = 6+isNatVars*(2); % isNatVars*(4+2+3)
for jF=1:nF,FLM(jF,:)={sprintf('FLM%.0f',jF),0};end

%% Lagrange multipliers for G constraints
nG = 4+isNatVars*(2); % isNatVars*(2+2+2)
for jG=1:nG,GLM(jG,:)={sprintf('GLM%.0f',jG),0};end
...
% fprintf('\nSolving for Ramsey Optimal Policy...\n')
% 
% %% Run RMS function to get symbolic matrices
% [RMSmat,RMSk_t]=Ramsey(y_t,csi_t,U,F,G,beta,S,y_ss,FLM_ss,GLM_ss,yLogIdx);
% 
% %% Fill RMS matrices with steady state and parameter vectors
% for j=1:nx
%     eval([x(j).name,'_ss = x1(',int2str(j),');'])
% end
% matnames = fieldnames(RMSmat);
% for j=1:length(matnames)
%     RMSmat.(matnames{j}) = reshape(eval(RMSmat.(matnames{j})(:)),size(RMSmat.(matnames{j})));
% end
% 
% %% Solve the REE
% [REE.Ramsey,RMSz_t] = RamseySolveREE(RMSmat,RMSk_t);
...
%% Alternative rule: TaylorNoNat
Rule.TaylorNoNat = phi_pi*hPi_t+phi_y/4*hY_t+xi_i_t-hRd_t;
...
%% Alternative rule: TaylorNoNatSP
% for j=25:25:100
%     phi_omega = j/100;
%     Rule.(sprintf('TaylorNoNatSP%.0f',j)) = phi_pi*hPi_t+phi_y/4*hY_t-phi_omega*homegatil_t+xi_i_t-hRd_t;
% end

%% Alternative rule: TaylorNoNatB
% % % if elast_omega_b_TGT==0
% %     phi_b = [-0.5,-0.25,0.25,0.5];
% % else
% %     phi_b = [-1,-0.5,0.5,1]*elast_omega_b_TGT;
% % end
% phi_b = [-0.10,-0.05,0.05,0.10];
% for jphi=1:length(phi_b)
%     RuleName = 'TaylorNoNatB';
%     if phi_b(jphi)<0
%         if phi_b(jphi)==-0.005
%             RuleName = sprintf('%sm%02.0f',RuleName,-phi_b(jphi)*1000);
%         else
%             RuleName = sprintf('%sm%.0f',RuleName,-phi_b(jphi)*100);
%         end
%     elseif phi_b(jphi)>0
%         if phi_b(jphi)==0.005
%             RuleName = sprintf('%sp%02.0f',RuleName,phi_b(jphi)*1000);
%         else
%             RuleName = sprintf('%sp%.0f',RuleName,phi_b(jphi)*100);
%         end
%     end
%     Rule.(RuleName) = phi_pi*hPi_t+phi_y/4*hY_t+phi_b(jphi)/4*hb_t+xi_i_t-hRd_t;
% end
...
Rule.Taylor = hRrdn_t+phi_pi*hPi_t+phi_y/4*(hY_t-hYn_t)+xi_i_t-hRd_t;

%% Alternative rule: TaylorSP
for j=[25:25:100,300,600,900,1200]
    phi_omega = j/100;
    Rule.(sprintf('TaylorSP%.0f',j)) = ...
...
for jphi=1:length(phi_b)
    ...
    Rule.(RuleName) = ...
        hRrdn_t+phi_pi*hPi_t+phi_y/4*(hY_t-hYn_t)+phi_b(jphi)/4*hb_t+xi_i_t-hRd_t;
end
...
%         irf(idxRrbn,:,:) = irf(idxRrdn,:,:)+irf(idxomegatiln,:,:);
%         % add cb (natural)
...
for j=1:ncsi
    VarShocks{j} = ShockSize*diag(ismember(csi,csi{j}));
    for jPol=1:nPol
